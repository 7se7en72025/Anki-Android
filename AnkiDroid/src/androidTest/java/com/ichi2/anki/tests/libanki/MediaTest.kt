...
class MediaTest : InstrumentedTest() {
    ...
    fun testDeckIntegration() {
        ...
        var f = testCol.newNote(testCol.notetypes.get(testCol.defaultsForAdding().notetypeId)!!)
        f.setField(0, "one")
        f.setField(1, "<img src='fake.png'>")
        testCol.addNote(f)
        // and one which references a non-existent file
        f = testCol.newNote(testCol.notetypes.get(testCol.defaultsForAdding().notetypeId)!!)
        f.setField(0, "one")
